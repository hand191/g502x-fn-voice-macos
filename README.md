...
版本 `0.3.2 (build 5)` 已完成普通插入光标、普通文本选区和 Chrome 地址栏全选替换复验。结果均为：松开后停止、文字只提交一次、光标与鼠标指针不移动。详细记录见 [docs/validation.md](docs/validation.md)。
...
3. 把光标放进可输入文字的位置；需要替换时，直接选中原文字。
4. 按住 G6 说话，松开 G6 结束；有选区时由输入法原位替换，连续两次使用之间建议留约 1 秒。
